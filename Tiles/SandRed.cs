...
{
    /*public class SandRedEntity : ModTileEntity
    {
        ...
        public override bool ValidTile(int i, int j)
        {
            ...
            return tile.active() && tile.type == mod.TileType<SandRed>();// && tile.frameX == 0 && tile.frameY == 0
        }
	...
	public class SandRed : ModTile
	{
		...
		public override void SetDefaults()
		{
			...
			drop = mod.ItemType("SandRedItem");
            AddMapEntry(new Color(200, 100, 100));
            dustType = 53;
            soundType = 38;
            //minPick = 40;
        }
        ...
        /*public override void ModifyLight (int i, int j, ref float r, ref float g, ref float b)
		{
			...
			//Main.NewText("Frame Counter" + frameCounter);
			if (i > 10 && i < Main.maxTilesX - 10 && j > 10 && j < Main.maxTilesY - 10)
            {
                //Down
                if (WorldGen.EmptyTileCheck((int)i, (int)i, (int)j + 1, (int)j + 1))
                {
                    WorldGen.PlaceTile((int)i, (int)j + 1, mod.TileType<Tiles.SandRed>(), true, true);
                ...
                else //If now down then this
                {
                    switch (WorldGen.genRand.Next(2)) //choose to fall downleft or down right
                    {
                        case 0://Down left, and then down right if fail
                            ...
                            {
                                WorldGen.PlaceTile((int)i - 1, (int)j + 1, mod.TileType<Tiles.SandRed>(), true, true);
